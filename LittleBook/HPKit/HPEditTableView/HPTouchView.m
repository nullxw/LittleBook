...
const float kHPTouchViewTouchLimitationTime = 0.5;
const float kHPTouchViewTouchTolerantRadius = 10.0;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchTime > kHPTouchViewTouchLimitationTime && tolerantRadius <= kHPTouchViewTouchTolerantRadius) {
        if (_delegate && [_delegate respondsToSelector:@selector(touchView:longPressAtPoint:)]) {
            [_delegate touchView:self longPressAtPoint:touchPoint];
        }
        return;
    }
