...
@interface LBImageFilterManager()
{
    dispatch_queue_t _queue;
}
@end

@implementation LBImageFilterManager

+ (LBImageFilterManager *)defaultManager
{
    CREATE_SINGLETON_INSTANCE([[LBImageFilterManager alloc] init]);
}
...
- (id)init
{
    if (self = [super init]) {
        ...
        const char *label = [[@"LBImageFilterManagerQueue" dataUsingEncoding:NSUTF8StringEncoding] bytes];
